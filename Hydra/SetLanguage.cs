...
namespace Hydra
{
    public class SetLanguage
    {
        ...
        public static void OnJoin(JoinEventArgs args)
        {
            ...
            {
                if (PortugueseCNCC.Where(p => p.ToLowerInvariant().Contains(player.Country.ToLowerInvariant())).ToString().Count() >= 1)
                    TSPlayerB.PlayerLanguage[player.Index] = TSPlayerB.Language.Portuguese;
                else if (EnglishCNCC.Where(p => p.ToLowerInvariant().Contains(player.Country.ToLowerInvariant())).ToString().Count() >= 1)
                    TSPlayerB.PlayerLanguage[player.Index] = TSPlayerB.Language.English;
                else if (SpanishCNCC.Where(p => p.ToLowerInvariant().Contains(player.Country.ToLowerInvariant())).ToString().Count() >= 1)
